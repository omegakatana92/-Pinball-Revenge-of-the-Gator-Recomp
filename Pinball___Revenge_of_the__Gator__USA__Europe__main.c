...
static char* show_open_rom_dialog(void) {
    char filepath[MAX_PATH] = {0};
    OPENFILENAME ofn = {0};
    ofn.lStructSize = sizeof(ofn);
    ofn.hwndOwner = NULL;
    ofn.lpstrFilter = "Game Boy ROMs\0*.gb;*.gbc;*.sgb\0All Files\0*.*\0";
...
int Pinball___Revenge_of_the__Gator__USA__Europe__main(int argc, char* argv[]) {
    ...
    if (!rom_path) {
#ifdef _WIN32
        rom_path = show_open_rom_dialog();
#endif
        if (!rom_path) {
            fprintf(stderr, "No ROM file selected. Exiting.\n");
            return 1;
        }
    }

    {
        ...
        long rom_len = ftell(rom_file);
        fseek(rom_file, 0, SEEK_SET);
        if (rom_len <= 0) {
            fprintf(stderr, "ROM file is empty: %s\n", rom_path);
            fclose(rom_file);
            return 1;
        }
        rom_data = (const uint8_t*)malloc((size_t)rom_len);
        if (!rom_data) {
            fprintf(stderr, "Failed to allocate memory for ROM\n");
            fclose(rom_file);
            return 1;
        }
        if (fread((void*)rom_data, 1, (size_t)rom_len, rom_file) != (size_t)rom_len) {
    ...
    GBConfig runtime_config = *Pinball___Revenge_of_the__Gator__USA__Europe__default_config();
    ...
    if (rom_size > 0x143) {
        ...
        runtime_config.cartridge_supports_cgb = (cgb_flag == 0x80 || cgb_flag == 0xC0);
        runtime_config.cartridge_requires_cgb = (cgb_flag == 0xC0);
    }
    ...
    if (differential_mode) {
        ...
        gb_context_set_save_id(generated_ctx, "Pinball___Revenge_of_the__Gator__USA__Europe_");
        gb_context_set_save_id(interpreted_ctx, "Pinball___Revenge_of_the__Gator__USA__Europe_");
        if (serial_stdout) {
            generated_ctx->callbacks.on_serial_byte = gbrt_serial_stdout;
        }
        Pinball___Revenge_of_the__Gator__USA__Europe__init(generated_ctx, rom_data, rom_size);
        Pinball___Revenge_of_the__Gator__USA__Europe__init(interpreted_ctx, rom_data, rom_size);
        if (data_mod_file) {
    ...
    gb_context_set_save_id(ctx, "Pinball___Revenge_of_the__Gator__USA__Europe_");
    ...
    Pinball___Revenge_of_the__Gator__USA__Europe__init(ctx, rom_data, rom_size);
    if (data_mod_file) {
    ...
    const GBPortMetadata port_metadata = {
        ...
        .game_id = "Pinball___Revenge_of_the__Gator__USA__Europe_",
        .game_title = "Pinball___Revenge_of_the__Gator__USA__Europe_",
        .rom_sha256 = "d84a2382b358d83a0891d40b2de65494f060d8be1a048d5aa8721282218b04b6",
        .rom_size = 65536u,
    };
    ...
    Pinball___Revenge_of_the__Gator__USA__Europe__run(ctx);
...
int main(int argc, char* argv[]) {
    return Pinball___Revenge_of_the__Gator__USA__Europe__main(argc, argv);
}
